...
public class ZoomPan20 implements ZoomPanable
{
	...
	private ZoomPan.ZoomPanBehaviour zoomPanType= ZoomPan.ZoomPanBehaviour.BOTH_DIRECTIONS; //By default, zooming/panning is in x and y
...
//		return getDispToCoord(new PVector(zoomPanState.aContext.mouseX,zoomPanState.aContext.mouseY));
	...
	*/
	...
	{    
		...
		double centreX = (zoomPanState.getPApplet().width*(1-zoomPanState.zoomScale))/2;
		double centreY = (zoomPanState.getPApplet().height*(1-zoomPanState.zoomScale))/2;

		zoomPanState.trans = new AffineTransform();
		zoomPanState.iTrans = new AffineTransform();

		//scale depending on the type
		if (zoomPanType== ZoomPan.ZoomPanBehaviour.BOTH_DIRECTIONS){
		...
		else if (zoomPanType== ZoomPan.ZoomPanBehaviour.VERTICAL_ONLY){
		...
		else if (zoomPanType== ZoomPan.ZoomPanBehaviour.HORIZONTAL_ONLY){
